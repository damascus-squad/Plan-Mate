...
class ManageAuditLogUseCase(
    ...
    suspend fun saveLog(history: History) = auditLogsRepository.saveLog(history)

    suspend fun getTaskLogs(taskId: UUID): List<History> {
        return auditLogsRepository.getLogsByTaskId(taskId)
            .takeIf { it.isNotEmpty() }
            ?: throw NoLogException("No history found for the task: $taskId")
    }

    suspend fun getProjectLogs(projectId: UUID): List<History> {
