...
class ManageAuditLogUseCaseTest {
    private lateinit var auditLogRepository: AuditLogsRepository
    private lateinit var manageAuditLogUseCase: ManageAuditLogUseCase
    ...
    fun setup() {
        ...
        manageAuditLogUseCase = ManageAuditLogUseCase(auditLogRepository)
    }
    ...
    fun `should return action log when history is not empty`() = runTest {
        ...
        // When
        val historyLog = manageAuditLogUseCase.getProjectLogs(projectId)
    ...
    fun `should throw NoHistoryException when no log are found for the given project id`() = runTest {
        ...
        assertThrows<NoLogException> {
            manageAuditLogUseCase.getProjectLogs(projectId)
        }
    }
    ...
    fun `should throw NoHistoryException when no log are found for the given task id`() = runTest {
        ...
        // When & Then
        assertThrows<NoLogException> {
            manageAuditLogUseCase.getTaskLogs(taskId)
        }
    }
