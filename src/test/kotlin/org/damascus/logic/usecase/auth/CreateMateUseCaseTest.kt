...
class CreateMateUseCaseTest {
    ...
    @Test
    fun `createMate should successfully create mate when admin credentials are valid`()= runTest {
        // Given
        ...
        coEvery { authRepo.createMate(adminUser, newUsername, newPassword) } returns newMateUser

        // When
        val result = createMateUseCase(adminUser, newUsername, newPassword)

        // Then
        coVerify { authRepo.createMate(adminUser, newUsername, newPassword) }
        assertThat(result.isSuccess).isTrue()
    }

    @Test
    fun `createMate should return failure when repository throws exception`() =runTest {
        // Given
        val adminUser = User(UUID.randomUUID(), "admin", UserRole.ADMIN)
        val newUsername = "newmate"
        val newPassword = "password123"
        coEvery { authRepo.createMate(adminUser, newUsername, newPassword) } throws InvalidCredentialsException()
